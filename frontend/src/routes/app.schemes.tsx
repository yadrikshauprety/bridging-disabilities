...
export const Route = createFileRoute("/app/schemes")({
  head: () => ({ meta: [{ title: "Scheme Entitlement Engine — Udaan" }] }),
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-20">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-4xl font-black tracking-tight">🏛️ {t("Scheme Entitlement Engine")}</h1>
          <p className="text-muted-foreground mt-1 text-lg italic">{t("\"What am I owed?\" — Real-time AI matching for Indian government schemes.")}</p>
        </div>
      ...
      <div className="grid lg:grid-cols-[1fr_2fr] gap-8 items-start">
        ...
        <div className="space-y-6">
          <section className="rounded-3xl border-2 border-border bg-card p-6 shadow-soft space-y-6">
            <h2 className="font-black text-xl flex items-center gap-2">🔍 {t("Your Profile")}</h2>
            <div className="space-y-4">
              <div>
                <label className="text-xs font-black uppercase tracking-widest text-muted-foreground block mb-2">{t("Disability Type")}</label>
                <select 
                ...
                >
                  <option>{t("Locomotor")}</option>
                  <option>{t("Visual Impairment")}</option>
                  <option>{t("Hearing Impairment")}</option>
                  <option>{t("Speech & Language")}</option>
                  <option>{t("Intellectual Disability")}</option>
                  <option>{t("Multiple Disabilities")}</option>
                </select>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-black uppercase tracking-widest text-muted-foreground block mb-2">{t("Age")}</label>
                  <input 
                    type="number"
                    value={profile.age}
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="T">{t("Other")}</option>
                  </select>
                </div>
              </div>
              <div>
                <label className="text-xs font-black uppercase tracking-widest text-muted-foreground block mb-2">{t("State")}</label>
                <input 
                  value={profile.state}
              ...
              <div>
                <label className="text-xs font-black uppercase tracking-widest text-muted-foreground block mb-2">{t("Income Group")}</label>
                <select 
                  value={profile.income}
                  onChange={e => setProfile({...profile, income: e.target.value})}
                  className="w-full rounded-xl border-2 border-border bg-background p-3 font-bold"
                >
                  <option value="BPL">{t("BPL (Below Poverty Line)")}</option>
            ...
            >
              {loading ? t("SEARCHING...") : t("FIND ENTITLEMENTS →")}
            </button>
          </section>
                  ...
                  <div key={i} className="group relative rounded-3xl border-2 border-border bg-card p-6 md:p-8 hover:border-primary transition-all shadow-sm hover:shadow-xl animate-in fade-in slide-in-from-bottom-8" style={{ animationDelay: `${i * 100}ms` }}>
                    ...
                    <div className="grid md:grid-cols-2 gap-6 mb-6">
                      <div className="space-y-1">
                        <span className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">{t("Benefit")}</span>
                        <p className="font-bold text-lg text-foreground">{s.benefit}</p>
                      </div>
                      <div className="space-y-1">
                        <span className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">{t("Eligibility")}</span>
                    ...
                    <div className="bg-muted/30 rounded-2xl p-5 mb-6 border border-border/50">
                      <span className="text-[10px] font-black uppercase text-muted-foreground tracking-widest block mb-2">{t("How to Apply")}</span>
                      <p className="font-medium text-sm leading-relaxed">{s.howToApply}</p>
                    </div>

                    <div className="flex items-center justify-between">
                      <div className="flex gap-2">
                        {s.tags?.map((t_tag: string, ti: number) => (
                          <span key={ti} className="text-[9px] font-black bg-primary/5 text-primary px-2 py-0.5 rounded border border-primary/10">{t_tag}</span>
                        ))}
                      </div>
                      <a 
                        href={s.sourceUrl} 
