...
type Interview = { id: string; candidateId: string; jobTitle: string; timestamp: string; transcript: { q: string; a: string; gesture?: string }[]; decisionStatus?: "applied" | "selected" | "rejected" | "round2"; decisionAt?: string };
type DeiAnalytics = {
  employerId: string;
...
function EmployerPortal() {
  ...
  const employerId = (localStorage.getItem("db_user_id") || "emp_1").toLowerCase();
  ...
  async function loadInterviews() {
    setLoadingInt(true);
    try {
      console.log("Loading interviews for:", employerId);
  ...
  async function moveToRound2(int: Interview) {
    ...
    try {
      ...
      if (sError || !session) throw new Error(`Supabase Session Error: ${sError?.message || 'Failed to create session'}`);
      ...
      const { error: nError } = await supabase.from("notifications").insert([
        ...
        { 
          ...
          session_id: session.id, 
          message: `Congratulations! You've been moved to Round 2 for ${int.jobTitle}. Join Meeting: ${meetLink}` 
        }
      ]);
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <p className="text-xs font-black uppercase tracking-widest text-blue-600">Active Candidates</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {interviews.slice(0, 3).map(int => (
                  <div key={int.id} className="bg-gray-50 p-5 rounded-2xl border border-gray-200">
                    <div className="flex justify-between items-start mb-3">
                      <h4 className="font-black text-gray-900 truncate pr-2" title={int.jobTitle}>{int.jobTitle}</h4>
                    </div>
                    <p className="text-[11px] text-gray-500 font-medium mb-4 truncate" title={int.candidateId}>{int.candidateId}</p>
                    <button 
                      onClick={() => { setTab("applications"); }}
